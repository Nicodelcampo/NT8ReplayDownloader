...
namespace NinjaTrader.Gui.NinjaScript
{
 ...
 {
  const string Build="EDGE_NT8RD_HARDENED_V3_AUTOCONVERT",Upstream="d4a9c7aa2048846ab6e6aa45b8f23530c5d60af3",Verified="8.1.8.2",NoData="no market replay data";
  ...
  void Convert(Instrument i,string contract,string cdir,string pq,string rp,string py)
  {string key=contract.Replace(' ','_');double tick=i.MasterInstrument.TickSize;
   string args=string.Format(CultureInfo.InvariantCulture,"tools\\l2_intake_sweep.py --csv-dir \"{0}\" --base \"{1}\" --instrument \"{2}\" --contract \"{3}\" --tick-size {4} --downloader-id \"{5}\" --log \"{6}\" --out \"{7}\"",
    Path.Combine(cdir,i.FullName),Path.Combine(pq,key),i.MasterInstrument.Name,contract,tick.ToString("R",CultureInfo.InvariantCulture),Build,Path.Combine(pq,"intake_logs",key+".jsonl"),Path.Combine(pq,"continuity",key));
   Log("CONVERT "+contract+" -> "+Path.Combine(pq,key)+" (running EdgeLab sweep)");
   try{ProcessStartInfo si=new ProcessStartInfo(py,args){WorkingDirectory=rp,UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true};
    using(Process p=Process.Start(si)){Task<string> err=p.StandardError.ReadToEndAsync();string o=p.StandardOutput.ReadToEnd();p.WaitForExit();string e=err.Result;
     foreach(string ln in o.Split(new[]{'\n'},StringSplitOptions.RemoveEmptyEntries)){string t=ln.Trim();if(t.StartsWith("\"conversion_failures")||t.StartsWith("\"boundaries_checked")||t.StartsWith("\"pass_count")||t.StartsWith("\"review_count")||t.StartsWith("\"fail_count")||t.StartsWith("\"session"))Log("  "+t.TrimEnd(','));}
     if(p.ExitCode==0)Log("CONVERT PASS "+contract);else{Log("CONVERT FAIL "+contract+" exit="+p.ExitCode+" (see "+Path.Combine(pq,"intake_logs",key+".jsonl")+")");if(e.Length>0){string[] el=e.Trim().Split('\n');Log("  "+el[el.Length-1].Trim());}}}}
