...
namespace NinjaTrader.Gui.NinjaScript
{
 ...
 public class EdgeLabReplayDownloaderWindow:NTWindow,IWorkspacePersistence
 {
  ...
  // Same call NRDToCSV makes (MarketReplay.DumpMarketDepth, public static in 8.1.8.2), same date shift (+1 day)
  // and same output layout (<root>/<Instrument.FullName>/<yyyyMMdd>.csv). Only runs on a .nrd that already
  // passed the depth check. The CSV is validated (non-empty, starts with an L1/L2 record) before it is recorded;
  // a bad export is quarantined, never left in place looking valid.
  ...
  Task Enqueue(Instrument i,string contract,string cdir,string pq,string rp,string py,bool full)
  {lock(convLock){convQ=convQ.ContinueWith(_=>Convert(i,contract,cdir,pq,rp,py,full));return convQ;}}
  static string JsonValue(string line){int c=line.IndexOf(':');return c<0?"":line.Substring(c+1).Trim().TrimEnd(',').Trim('"');}
  void Convert(Instrument i,string contract,string cdir,string pq,string rp,string py,bool full)
  {string key=contract.Replace(' ','_');double tick=i.MasterInstrument.TickSize;string logPath=Path.Combine(pq,"intake_logs",key+".jsonl");
   string args=string.Format(CultureInfo.InvariantCulture,"tools\\l2_intake_sweep.py --csv-dir \"{0}\" --base \"{1}\" --instrument \"{2}\" --contract \"{3}\" --tick-size {4} --downloader-id \"{5}\" --log \"{6}\" --out \"{7}\"{8}",
    Path.Combine(cdir,i.FullName),Path.Combine(pq,key),i.MasterInstrument.Name,contract,tick.ToString("R",CultureInfo.InvariantCulture),Build,logPath,Path.Combine(pq,"continuity",key),full?"":" --no-continuity");
   if(full)Log("CONVERT "+contract+" -> "+Path.Combine(pq,key)+" (final pass + continuity)");
   try{ProcessStartInfo si=new ProcessStartInfo(py,args){WorkingDirectory=rp,UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true};
    using(Process p=Process.Start(si)){Task<string> err=p.StandardError.ReadToEndAsync();string o=p.StandardOutput.ReadToEnd();p.WaitForExit();string e=err.Result;string sess=null;
     foreach(string ln in o.Split(new[]{'\n'},StringSplitOptions.RemoveEmptyEntries))
     {string t=ln.Trim();
      if(t.StartsWith("\"session")){sess=JsonValue(t);continue;}
      if(t.StartsWith("\"conversion_status")&&sess!=null){Log("PARQUET "+sess+" "+contract+" "+JsonValue(t));sess=null;continue;}
      if(full&&(t.StartsWith("\"conversion_failures")||t.StartsWith("\"boundaries_checked")||t.StartsWith("\"pass_count")||t.StartsWith("\"review_count")||t.StartsWith("\"fail_count")))Log("  "+t.TrimEnd(','));}
     if(p.ExitCode==0){if(full)Log("CONVERT PASS "+contract);}
     else{Log("CONVERT FAIL "+contract+" exit="+p.ExitCode+" (see "+logPath+")");if(e.Length>0){string[] el=e.Trim().Split('\n');Log("  "+el[el.Length-1].Trim());}}}}
   catch(Exception ex){Log("CONVERT ABSTAIN "+contract+" "+ex.GetType().Name+": "+ex.Message);}}
  void Start()
  {if(running)return;if(Version()!=Verified){Log("ABSTAIN: unsupported NT Core "+Version()+"; verified "+Verified);return;}string cd;HdsClient h=Client(out cd);if(h==null){Log("ABSTAIN: "+cd);return;}HashSet<string> set=new HashSet<string>(StringComparer.OrdinalIgnoreCase);foreach(string x in contracts.Text.Split(new[]{';'},StringSplitOptions.RemoveEmptyEntries)){string c=x.Trim().ToUpperInvariant();if(!ContractRx.IsMatch(c)){Log("ERROR exact contract required: "+c);return;}set.Add(c);}if(set.Count<1||set.Count>20){Log("ERROR: 1-20 contracts required");return;}DateTime a,b;if(!DateTime.TryParseExact(from.Text.Trim(),"yyyy-MM-dd",CultureInfo.InvariantCulture,DateTimeStyles.None,out a)||!DateTime.TryParseExact(to.Text.Trim(),"yyyy-MM-dd",CultureInfo.InvariantCulture,DateTimeStyles.None,out b)||a>b||b.Date>=DateTime.Now.Date||(b-a).TotalDays+1>120){Log("ERROR: valid past range <=120 days required");return;}int wait;if(!int.TryParse(delay.Text.Trim(),out wait)||wait<3||wait>60){Log("ERROR delay 3-60");return;}string cdir=csvdir.Text.Trim();if(cdir.Length>0&&!Path.IsPathRooted(cdir)){Log("ERROR: CSV export root must be an absolute path");return;}string pq=pqroot.Text.Trim(),rp=repo.Text.Trim(),py=pyexe.Text.Trim();if(pq.Length>0){if(cdir.Length==0){Log("ERROR: auto-conversion needs the CSV export root");return;}if(!Path.IsPathRooted(pq)){Log("ERROR: parquet root must be an absolute path");return;}if(!File.Exists(Path.Combine(rp,"tools","l2_intake_sweep.py"))){Log("ERROR: tools\\l2_intake_sweep.py not found in EdgeLab repo");return;}if(!File.Exists(py)){Log("ERROR: python exe not found");return;}}cancel=false;Running(true);Thread t=new Thread(()=>Run(h,new List<string>(set),a.Date,b.Date,wait,cdir,pq,rp,py));t.IsBackground=true;t.Start();}
  void Run(HdsClient h,List<string> list,DateTime a,DateTime b,int wait,string cdir,string pq,string rp,string py)
  {int ok=0,skip=0,none=0,fail=0;try{foreach(string contract in list){Instrument i=Instrument.GetInstrument(contract,true);if(i==null){Manifest(contract,a,"ABSTAIN_INSTRUMENT_NOT_FOUND","",null,"","NOT_REQUESTED","instrument absent");fail++;continue;}for(DateTime d=a;d<=b;d=d.AddDays(1)){if(cancel)return;string path=PathFor(contract,d);DepthCheck before=Check(path);if(before.Pass){string sha,hd;if(!Hash(path,out sha,out hd)){Manifest(contract,d,"ABSTAIN_HASH_FAILED",path,before,"","NOT_REQUESTED",hd);fail++;continue;}Manifest(contract,d,"SKIP_EXISTING_VALID",path,before,sha,"NOT_REQUESTED",before.Detail);skip++;if(cdir.Length>0){ExportCsv(i,contract,d,cdir);if(pq.Length>0)Enqueue(i,contract,cdir,pq,rp,py,false);}continue;}if(File.Exists(path)){try{Log("QUARANTINE "+Quarantine(path));}catch(Exception ex){Manifest(contract,d,"ABSTAIN_QUARANTINE_FAILED",path,before,"","NOT_REQUESTED",ex.GetType().Name);fail++;continue;}}RequestResult rr=Request(h,i,d);if(rr.State==RequestState.Timeout||rr.State==RequestState.Cancelled){Manifest(contract,d,"ABSTAIN_REQUEST_TERMINAL",path,null,"",rr.State.ToString(),rr.Message);Log("TERMINAL "+rr.State+"; batch stopped");return;}if(IsNoData(rr)&&!File.Exists(path)){Manifest(contract,d,"NO_SERVER_DATA",path,null,"",rr.State.ToString(),"server no data");none++;}else{DepthCheck after=Stable(path,TimeSpan.FromSeconds(30));if(after.Pass){string sha,hd;if(Hash(path,out sha,out hd)){Manifest(contract,d,"ACQUIRED_VALID_DEPTH",path,after,sha,rr.State.ToString(),after.Detail);Log("PASS "+d.ToString("yyyy-MM-dd")+" "+contract+" "+after.Detail);ok++;if(cdir.Length>0){ExportCsv(i,contract,d,cdir);if(pq.Length>0)Enqueue(i,contract,cdir,pq,rp,py,false);}}else{Manifest(contract,d,"ABSTAIN_HASH_FAILED",path,after,"",rr.State.ToString(),hd);fail++;}}else{string sha="",hd;if(File.Exists(path))Hash(path,out sha,out hd);Manifest(contract,d,"ABSTAIN_INVALID_DOWNLOAD",path,after,sha,rr.State.ToString(),after.Detail);fail++;}}if(d<b&&!cancel)Thread.Sleep(wait*1000);}if(pq.Length>0&&!cancel){Log("CONVERT "+contract+": waiting for per-day conversions");Enqueue(i,contract,cdir,pq,rp,py,true).Wait();}}}catch(Exception ex){Log("ABSTAIN UNHANDLED "+ex.GetType().Name);}finally{Running(false);Log(string.Format(CultureInfo.InvariantCulture,"-- finished {0} acquired, {1} existing, {2} no-data, {3} failed --",ok,skip,none,fail));}}
